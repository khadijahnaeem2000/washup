...
class Csr_dashboardController extends Controller
{
    ...
    public function update_order(Request $request)
    {
        ...
        if($data->status_id2==16){ //CHECKING LAST STATUS OF ORDER : IS IT A CANCELLED ORDER??
            // $request['status_id2']  = NULL;
            $request['status_id2']  = $this->get_last_order_status($id);
            $var                    = new Order_history();
            $var->order_id          = $id;
            $var->created_by        = Auth::user()->id;
            $var->status_id         =  $request['status_id'];
            $var->save();
        }
        if($temp==16){
            $request['status_id']       = $data->status_id;
            $request['status_id2']      = 16;
        }
        ...
        if(($data)){
            ...
            if ($validator->passes()) {
                ...
                if($inputs['status_id']==1){
                    // for reschedulling: changed to null
                    if($temp==16){
