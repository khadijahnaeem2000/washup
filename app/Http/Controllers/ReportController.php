    ...
    public function fetch_riders(Request $request)
    {
        if($request->ajax()){
            ...
            $riders     = DB::table('hub_has_riders')
                            ...
                            ->all();
            if ($riders){
                $data   = view('reports.ajax-riders',compact('riders'))->render();
    ...
    public function list(Request $request)
    {
        ...
        if(!($orders->isEmpty())){
                ...
                foreach ($orders as $key => $value) {
                    ...
                    $value->s_loc               = $this->fn_order_count($hub_id, $rider_id, $dt);
                    $value->s_polybags          = $this->fn_count_polybags($hub_id, $rider_id, $dt,'all');
                    $value->s_dist              = $this->fn_calc_kms($hub_id, $rider_id, $dt,'req_dist'); // required distance

                    $value->a_loc               = $this->fn_order_count($hub_id, $rider_id, $dt);
                    $value->a_polybags          = $this->fn_count_polybags($hub_id, $rider_id, $dt,'scan');
                    $value->a_dist              = $this->fn_calc_covered_kms($rider_id, $dt); //covered distance

                    $value->receiving           = $this->fn_calc_receiving($rider_id, $dt);
                ...
                // dd($rec);
                if(!($orders->isEmpty())){
                    $details    = view('reports.report_table',
                                      compact('rec'))
                                    ->render();
