...
class OrderController extends Controller
{
   ...
   public function fetch_items($order_id, $service_id){
    ...
    public function fn_get_all_services_amount($order_id){
        $data                   = "";
        $tot                    = 0;
        $all_services           =  DB::table('orders')
                                    ->leftjoin('order_has_services', 'order_has_services.order_id', '=', 'orders.id')
    ...
    public function fn_get_all_addons_amount($order_id){
        $amount                 = 0;
        $all_addons             =  DB::table('order_has_addons')
                                    ->leftjoin('order_has_items', 'order_has_items.id', '=', 'order_has_addons.ord_itm_id')
                                    ->where('order_has_addons.order_id',$order_id)
    ...
    public function confirm_pickup(Request $request){
        ...
        $dataa  = Order::find($request->order_id);
        

        if( (isset($dataa->status_id2)) && ( ($dataa->status_id2) == 16) ){
            return response([
                            'status'    => 'failed',
        ...
        try {
            ...
            $exception  = DB::transaction(function()  use ($request) {
                $tst    = 0;
                $data   = Order::where('status_id2', '!=' ,16)->find($request->order_id);
                $id     = $request->order_id;
                ...
                if( $tst == 1){
                 ...
                 // get and sum all service / items rates
                 $service_tot              = $this->fn_get_all_services_amount($id);

                 // get and sum all addon rates
                 $addon_tot                = $this->fn_get_all_addons_amount($id);
    ...
    public function fetch_dropoff($rider_id,$order_id,$status_id = 2){
        ...
        if($orders != null){
            ...
            $orders->wallet_amount     = (($wallet->wallet == null) ) ? 0 : $wallet->wallet;  //600
            ...
            if(($orders->wallet_amount) > 0){
                $orders->payable_amount   = abs($orders->wallet_amount - $orders->order_amount);
            }else{
                $orders->payable_amount   = ((abs($orders->wallet_amount)) + $orders->order_amount);
            }
           
            // $orders->payable_amount   = ($orders->wallet_amount + $orders->order_amount);
            // $orders->payable_amount   = ($orders->wallet_amount );
    ...
    public function store_payment(Request $request){
        ...
        // BEGIN::check is payment received against this order
            $wllt                       = Customer_has_wallet::select('id')
            ...
            if((isset($wllt->id))){
                return response([
                    'status'    => 'failed',
                    'error'     => "Payment has been received against this order!",
                ], 404);
            }
        // END::check is payment received against this order
        ...
        try {
            ...
            $exception = DB::transaction(function()  use ($request,$data) {
                ...
                // BEGIN::calculating order amount

                    // get and sum all service / items rates
                    $service_tot              = $this->fn_get_all_services_amount($request->order_id);

                    // get and sum all addon rates
                    $addon_tot                = $this->fn_get_all_addons_amount($request->order_id);
                ...
                // END::calculating order amount


                // BEGIN::delete previous transaction over this order 

                $dtl                            = ("order:" .$request->order_id);
                $cstmr_has_wllt                 = DB::table("customer_has_wallets")
                                                    ->where('customer_has_wallets.customer_id', '=', $data->customer_id)
                                                    ->where('customer_has_wallets.order_id', '=', $request->order_id)
                                                    ->where('customer_has_wallets.detail', '=', $dtl)
                                                    ->where('customer_has_wallets.out_amount', '>', 0)
                                                    ->delete();
                ...
                // BEGIN::store wallet amount as credit/ in_amount
                    ...
                    if( !(isset($data->ref_order_id)) ){  // if order is regular
                        $val->out_amount            = ($temp_amount + $vat_amount); // sum of all service, addon, items, vat, and delivery rates
                    }
    ...
    public function store_pay_rides(Request $request){
        ...
        $dataa                  = Payment_ride::select('customer_id')->find($request->order_id);

         // BEGIN::check is payment received against this ride
            $wllt               = Customer_has_wallet::select('id')
                                    ->where('customer_id', $dataa->customer_id)
                                    ->where('ride_id',$request->order_id)
                                    ->where('in_amount','>',0)
                                    ->first();
            ...
            if((isset($wllt->id))){
                return response([
                    'status'    => 'failed',
                    'error'     => "Payment has been received against this ride!",
                ], 404);
            }
        // END::check is payment received against this order
